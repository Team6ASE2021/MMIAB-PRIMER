...
#TODO: refactor to generate test data automatically
    ...
    def test_users_list_logged(self, test_client):
        test_client.post('/login', data={'email': 'example@example.com', 'password': 'admin'}, follow_redirects=True)
        response = test_client.get('/users')
        assert response.status_code == 200
        assert b'User List' in response.data
    ...
    def test_user_delete(self, test_client):
        ...
        response = test_client.post('/login', data={'email': data['email'], 'password': data['password']}, follow_redirects=True)
        assert response.status_code == 200

        new_user = UserModel.get_user_info_by_email(data['email'])

        response = test_client.get('users/' + str(new_user.id) + '/delete', follow_redirects=True)
        assert response.status_code == 200
    ...
    def test_user_blacklist_add(self, test_client):
        ...
        response = test_client.get('/user/blacklist/add/' + str(new_user.id), follow_redirects=True)
        assert response.status_code == 200
    ...
    def test_user_blacklist_remove(self, test_client):
        ...
        response = test_client.get('/user/blacklist/remove/' + str(new_user.id), follow_redirects=True)
        assert response.status_code == 200
